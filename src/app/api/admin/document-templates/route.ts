...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const doc = await DocumentTemplate.create({ title, category, fileName, mimeType, fileSize, fileData });
    return NextResponse.json(
      { _id: doc._id, title: doc.title, category: doc.category, fileName: doc.fileName, mimeType: doc.mimeType, fileSize: doc.fileSize, createdAt: doc.createdAt },
      { status: 201 }
    );
  } catch (err) {
