...
import { Calculator, MapPin, Plane, Ship, Truck, Zap } from "lucide-react";
...
// Static approximation for display only — this is a rough instant estimate,
// not a live FX rate. Base rates above are in USD.
const USD_TO_INR = 83;

const pickupCountries = ["India", "Other"] as const;

export default function RateCalculator() {
  const [type, setType] = useState<(typeof rates)[number]["value"]>("air");
  const [weight, setWeight] = useState("");
  const [pickupCountry, setPickupCountry] = useState<(typeof pickupCountries)[number] | null>(null);

  const selected = rates.find((r) => r.value === type)!;
  const parsedWeight = Number(weight) || 0;
  const estimateUsd = parsedWeight > 0 ? parsedWeight * selected.perKg + selected.handling : null;

  const currency = pickupCountry === "India" ? "INR" : "USD";
  const symbol = currency === "INR" ? "₹" : "$";
  const estimate =
    estimateUsd !== null ? (currency === "INR" ? estimateUsd * USD_TO_INR : estimateUsd) : null;
        ...
        <div className="grid grid-cols-2 gap-2">
          {pickupCountries.map((country) => (
            <button
              key={country}
              type="button"
              onClick={() => setPickupCountry(country)}
          ...
          {estimate !== null && (
            <div className="mt-4 rounded-xl bg-navy/5 p-4 dark:bg-white/5">
              ...
              <p className="mt-1 font-heading text-2xl font-bold text-navy dark:text-white">
                {symbol}{estimate.toFixed(2)} – {symbol}{(estimate * 1.15).toFixed(2)}
              </p>
